...
<%
   ...
   String sql = request.getParameter("sql"); //처음엔 null
%>
...
<% 
    ...
    if(sql != null && !sql.trim().equals("")) {  // sql이 null 이면 db와 연결x
    try {
	...
	rs2.next();
	int cnt = rs2.getInt(1); // 1번컬럼인 count(*)를 가져옴
%>
<table><tr><td colspan="<%=rsmt.getColumnCount() %>" style="text-align : right"> 조회된 레코드 갯수:<%=cnt %></td></tr>
<!-- 컬럼명 출력 -->
<tr>
